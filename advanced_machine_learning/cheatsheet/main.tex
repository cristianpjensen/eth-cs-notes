...
\begin{multicols*}{3}
    ...
    \textbf{JUSTIFY EVERY STEP IN DERIVATIONS!}
    ...
    $\lim_{n\to \infty} \lft( 1-\nicefrac{1}{n} \rgt)^n = \nicefrac{1}{e} \approx 0.368$.

    $\exp(x) = \sum_{n=0}^{\infty} \nicefrac{x^n}{n!}$.

    \scalebox{0.78}{$\mathrm{Cov}(\vec{x},\vec{y}) = \E[(\vec{x} - \E[\vec{x}]) \transpose{(\vec{y} - \E[\vec{y}])}] = \E[\vec{x}\transpose{\vec{x}}] - \E[\vec{x}]\transpose{\E[\vec{x}]}$.}
