...
Using the gradient descent update rule, we get \[
    \vec{g}_t \doteq \grad{f(\vec{x}_t)}{} = \frac{\vec{x}_t - \vec{x}_{t+1}}{\gamma},
\]
which gives the following bound
\begin{align*}
    f(\vec{x}_t) - f(\vec{x}^\star) & \leq \transpose{\vec{g}_t} (\vec{x}_t - \vec{x}^\star)                                                                                                                                                                                                     \\
                                    ...
                                    & = \frac{\gamma}{2} \|\vec{g}_t\|^2 + \frac{1}{2\gamma}\lft( \|\vec{x}_t-\vec{x}^\star\|^2 - \|\vec{x}_{t+1}-\vec{x}^\star\|^2 \rgt).
\end{align*}
Summing this up over the first $T$ iterations, we get an upper bound on the summed error,
\begin{align*}
    \sum_{t=0}^{T-1} (f(\vec{x}_t) - f(\vec{x}^\star)) & \leq \sum_{t=0}^{T-1} \transpose{\vec{g}_t}(\vec{x}_t - \vec{x}^\star)                                                                            \\
                                                       & = \frac{\gamma}{2} \sum_{t=0}^{T-1} \|\vec{g}_t\|^2 + \frac{1}{2\gamma} \lft( \|\vec{x}_0-\vec{x}^\star\|^2 - \|\vec{x}_T-\vec{x}^\star\|^2 \rgt) \\
                                                       & \leq \frac{\gamma}{2} \sum_{t=0}^{T-1} \|\vec{g}_t\|^2 + \frac{1}{2\gamma} \|\vec{x}_0-\vec{x}^\star\|^2.
\end{align*}
We can also use this to get a bound on the average error, \[
    \frac{1}{T} \sum_{t=0}^{T-1} (f(\vec{x}_t) - f(\vec{x}^\star)) \leq \frac{1}{T} \lft( \frac{\gamma}{2} \sum_{t=0}^{T-1} \|\vec{g}_t\|^2 + \frac{1}{2\gamma} \|\vec{x}_0-\vec{x}^\star\|^2 \rgt).
\]

\subsection{Lipschitz convex functions}

\marginnote{A function $f: \R\to \R$ is Lipschitz continuous if there exists an $M$, such that
    $\forall x,y\in\R:$ \[
...
\begin{proof}
    ...
    \begin{align*}
        \sum_{t=0}^{T-1} (f(\vec{x}_t) - f(\vec{x}^\star)) & \leq \frac{\gamma}{2} \sum_{t=0}^{T-1} \|\vec{g}_t\|^2 + \frac{1}{2\gamma} \|\vec{x}_0-\vec{x}^\star\|^2 \\
...
\begin{lemma}
    Let $f: \R^d \to \R$ be convex and differentiable, then $f$ is smooth with parameter $L$ if \[
...
\begin{proof}
    \begin{align*}
        f(\vec{x}_{t+1}) & \leq f(\vec{x}_t) + \transpose{\grad{f(\vec{x}_t)}{}} (\vec{x}_{t+1} - \vec{x}_t) + \frac{L}{2} \|\vec{x}_t-\vec{x}_{t+1}\|^2 \margintag{Smoothness.}                                                                             \\
                         & = f(\vec{x}_t) + \transpose{\grad{f(\vec{x}_t)}{}} \lft( - \frac{\grad{f(\vec{x}_t)}{}}{L} \rgt) + \frac{L}{2} \lft\| \frac{\grad{f(\vec{x}_t)}{}}{L} \rgt\|^2 \margintag{$\vec{x}_{t+1} - \vec{x}_t = -\grad{}f(\vec{x}_t){L}$.} \\
...
\begin{proof}
    Due to sufficient decrease, we can bound the squared gradients (which will be useful when bounding
    the vanilla analysis), \[
        \frac{1}{2L} \sum_{t=0}^{T-1} \| \grad{f(\vec{x}_t)}{} \|^2 \leq \sum_{t=0}^{T-1} f(\vec{x}_t) - f(\vec{x}_{t+1}) = f(\vec{x}_0) - f(\vec{x}_T).
    \]
...
Using this result, we can compute a bound on $T$ to achieve an error of $\epsilon$, \[
    f(\vec{x}_T) - f(\vec{x}^\star) \leq \frac{L}{2T} \|\vec{x}_0 - \vec{x}^\star \|^2 \leq \frac{R^2L}{2T}.
\]
...
derive a bound on the error from the vanilla analysis, TODO

\begin{theorem}
    Let $f: \R^d \to \R$ be convex and differentiable with a global minimum $\vec{x}^\star$.
    Furthermore, suppose that $f$ is smooth with parameter $L$ and strongly convex with parameter
    $\mu > 0$. Choosing $\gamma \doteq \frac{1}{L}$, gradient descent with arbitrary $\vec{x}_0$
...
$0.01$, as opposed to $50\cdot R^2 L$ in the smooth case.
