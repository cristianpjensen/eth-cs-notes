...
\vec{\phi}(x) = \transpose{\begin{bmatrix} 1 & x & x^2 \end{bmatrix}}.\]} but
...
function,\sidenote{Formally, $k(\vec{x},\vec{x}')=\transpose{\vec{\phi}(\vec{x})}
...
the period. \Cref{fig:gp-per-kernel} shows the periodic kernel function \wrt
$x'=0$. As can be seen, pairs of points $(x,x')$ with high covariance $k(x,x')$
have close values in all sampled function realizations. This makes sense, since
$k$ parametrizes the covariance.
...
\begin{itemize}
  \item \textit{Linear kernel}, \[
    ...
    .\]
    Its posterior can be seen in \Cref{fig:gp-posterior-linear};
  ...
  \item \textit{Gaussian kernel} (a.k.a. \textit{Squared Exponential} or
    ...
    .\]
    Its posterior can be seen in \Cref{fig:gp-posterior-gaussian};

  \item \textit{Exponential kernel}, \[
      k(\vec{x},\vec{x}') = \exp \lft( -\frac{\lVert \vec{x}-\vec{x}' \rVert}{\ell} \rgt)
    .\]
    Its posterior can be seen in \Cref{fig:gp-posterior-exponential}.
\end{itemize}
...
As can be seen in Figures \Cref{fig:gp-posterior,fig:posterior-exponential}, the
...
BLR, this is $O(dn^2)$.} and the space complexity of storing
$k(\mat{X},\mat{X})$ is $\Theta(n^2)$. The main approaches for accelerating GP
posterior computation are exploiting parallelism (GPU),\sidenote{This yields
a significant speedup, but does not address the cubic scaling in $n$.} local
