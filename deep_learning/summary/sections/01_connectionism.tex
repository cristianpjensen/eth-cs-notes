...
with $|\Xi| \leq n$ is linearly independent.\sidenote{This is a very weak condition.}
...
\begin{proof}
    ...
    \begin{enumerate}
        \item $\mathcal{V}^+$ and $\mathcal{V}^-$ are non-empty. Hence, $\vec{x}_{s+1}$ can be
              ...
              $\vec{w} \in \mathcal{V}$ such that $\transpose{\vec{w}} \vec{x}_{s+1} = 0$.\sidenote{Because
...
It turns out that after $s=2n$, there is a steep decrease in number of linear classifications,
...
%
% Function: \[
%     f[\mat{W}, \vec{v}](\vec{x}) = \transpose{\vec{v}} \phi(\mat{W}\vec{x}), \quad \mat{W} \in \R^{d \times n}, \vec{v} \in \R^d.
% \]

\subsection{Hopfield networks}

The Hopfield model \citep{hopfield1982neural} defines a parameterized energy function via
...
If we assume that the patterns have i.i.d. random signs and we look at the limit $n \to \infty$,
then we have \[
