...
\begin{multicols*}{3}
    ...
    \begin{itemize}
        ...
        \item Gaussian arithmetic:
              \begin{align*}
                   & \vec{x} \sim \mathcal{N}(\vec{\mu}_{\vec{x}}, \mat{\Sigma}_{\vec{x}}), \vec{y} \sim \mathcal{N}(\vec{\mu}_{\vec{y}}, \mat{\Sigma}_{\vec{y}}) \\
                   & \implies \vec{x} + \vec{y} \sim \mathcal{N}(\vec{\mu}_{\vec{x}} + \vec{\mu}_{\vec{y}}, \mat{\Sigma}_{\vec{x}} + \mat{\Sigma}_{\vec{y}})      \\
                   & \vec{x} \sim \mathcal{N}(\vec{\mu}, \mat{\Sigma}), \mat{M} \in \R^{m \times d}                                                               \\
                   & \implies \vec{y} = \mat{M} \vec{x} \sim \mathcal{N}(\mat{M} \vec{\mu}, \mat{M}\mat{\Sigma}\transpose{\mat{M}}).
              \end{align*}
        \item Sign function $\mathrm{sgn}(z) = \mathbb{1}\{ z \geq 0 \}$.
        \item Cosine angle $\cos \angle{(\vec{x},\vec{y})} = \frac{\transpose{\vec{x}} \vec{y}}{\| \vec{x} \| \|
                      \vec{y} \|}$.
        \item Gradients:
              \begin{align*}
                   & \vec{z} = \mat{W} \vec{x}                                        \\
                   & \implies \pdv{L}{\mat{W}} = \pdv{L}{\vec{z}} \transpose{\vec{x}} \\
                   & \vec{h} = f(\vec{z}) \text{ element-wise}                        \\
                   & \implies \pdv{L}{\vec{z}} = \pdv{L}{\vec{h}} \odot f'(\vec{z})   \\
                   & \pdv{L}{x_i} = \sum_{j=1}^{m} \pdv{L}{y_j} \pdv{y_j}{x_i} \\
                   & \pdv{\transpose{\vec{x}\mat{A}\vec{x}}}{\vec{x}} = (\mat{A}+\transpose{\mat{A}})\vec{x}.
              \end{align*}
        ...
        \item Probability review:
            \begin{align*}
                p(X \mid Y) & = \frac{p(Y \mid Y) p(X)}{p(Y)} \\
