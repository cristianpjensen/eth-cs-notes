...
each class.\sidenote{$\Delta^{k-1}$ is the $k$-dimensional probability simplex, which is the space
    of vectors where all elements are non-negative and sum to 1.} To fully parametrize the latent
...
with word occurrences, which induce topics of entire documents.\sidenote{If we were to associate
    ...
    contain.} Thus, the complete data will be a sequence of word-topic pairs for each document, \[
...
process.\sidenote{This is important so we can infer/reverse engineer a model from it.} For each
...
As a result, we can use the class-conditional word distribution, $p(w \mid z)$, to find similar
