...
\begin{lemma}[First-order convexity]
...
An example of a strictly convex function can be found in \Cref{fig:convexity}, while an example of a function that is not strictly convex can be found in \Cref{fig:non-strict-convexity}.
...
the primal and dual problem.\sidenote{This would mean that
$\min f_0(\vec{x}) = \max g(\vec{\lambda},\vec{\nu})$.} If all $f_i$ and $h_j$ are differentiable,
...
point\sidenote{A point $\tilde{\vec{x}}$ such that $f_i(\tilde{\vec{x}}) < 0$ for all $i=1,\ldots,m$.},
