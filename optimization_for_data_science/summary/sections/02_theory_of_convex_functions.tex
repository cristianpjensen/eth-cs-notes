...
stretched in length under the mapping $\vec{v}\mapsto\mat{A}\vec{v}$.
...
\begin{lemma}[Second-order convexity]
    ...
    Then, $f$ is convex if and only if (i) $\dom{f}$ is convex and (ii) we have \[
        \hess{f(\vec{x})}{} \succeq \mat{0}, \quad \forall \vec{x} \in \dom{f}. \margintag{\Ie, $\hess{f(\vec{x})}{}$ is positive semidefinite ($\mat{M}$ is positive semidefinite if $\transpose{\vec{x}}\mat{M}\vec{x} \geq 0$ for all $\vec{x}$ and $\transpose{\vec{x}}\mat{M}\vec{x} > 0$ for all $\vec{x} \neq \vec{0}$).}
    \]
\end{lemma}
